...
class Sales_topic_category_model extends CI_Model
{
     ...
     * @param unknown $salesId
     */
    public function getSalesProduct($salesId,$flag=false) {
    	
    	$this->db->select('sales_category_product.product_id,sales_category_product.category_id');
    	$this->db->from($this->table);
    	...
    	if ($flag) {
    		$salesArray = array();
    		foreach ($result->result() as $i=>$item) {
    			$salesArray[$item->category_id][] = $item->product_id;
    		}
    		return $salesArray;
    	}
    	return $result;
